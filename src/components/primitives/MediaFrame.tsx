...
export function MediaFrame({ clip }: MediaFrameProps) {
  ...
  return (
    ...
    >
      <div className="orbit-panel h-full rounded-[18px] shadow-card">
        <div className="orbit-panel-inner flex h-full flex-col overflow-hidden border border-s1-forest/40 bg-s1-ink">
          <div className="relative shrink-0">
            {hasVideo ? (
              ...
              <div className="flex aspect-[1920/1002] w-full flex-col items-center justify-center gap-3 bg-gradient-to-br from-s1-ink to-s1-deep text-center">
                ...
                <p className="max-w-[18ch] text-base text-s1-sand/90">
                  Production media slot reserved – assets ship with editor capture.
                </p>
              </div>
            )}
            {clip.media.poster ? (
          ...
          <figcaption className="flex flex-1 flex-col justify-center space-y-4 px-8 pb-8 pt-6">
            <div className="space-y-3">
              <h3 className="font-display text-3xl text-s1-sand">{clip.title}</h3>
              <p className="text-base text-s1-sand/80 leading-relaxed">{clip.description}</p>
            </div>
            <p className="text-sm text-s1-sand/60 pt-4 border-t border-white/5">{clip.caption}</p>
          </figcaption>
        </div>
      </div>
    </motion.figure>
